...
public class NeuralNetwork {
  ...
  /**
   * First layer is inputs (not perceptrons)
   * Proceeding layers are perceptrons
   * <p>
   * Ex: 3 layered (input, hidden, output) network will have
   * 2 perceptron layers (hidden, output)
   *
   * @param inputAmount  number of initial inputs
   * @param hiddenCount  Length = number of hidden layers,
   *                     Values = number of perceptrons in that layer
   * @param outputCount  number of output perceptrons
   * @param learningRate
   * @param isChild
   */
  public NeuralNetwork(int inputAmount, int[] hiddenCount,
          int outputCount, float learningRate, float mutationRate,
          boolean isChild) {
  ...
  /**
   * Calculate the sum of total inputs * weights + bias.
   * For forward outputting.
   *
   * @param inputsF
   * @return
   */
    ...
    for (int i = 0; i < layers.length; i++) {
      ...
      for (int x = 0; x < layers[i].length; x++) {
        ...
        if (i == 0) {
          ...
          inputForNextLayer[x] = layers[i][x].getOutput(inputForNextLayerTemp);
        }
      }
  ...
  /**
   * Print out the number of perceptrons in each layer,
   * Also the number of inputs it receives.
   */
